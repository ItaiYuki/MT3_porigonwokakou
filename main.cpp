...
const char kWindowTitle[] = "LE2D_06_イタイ_ユウキ";
struct Vector3 {
  float x;
  float y;
  float z;
...
// アフィン変換行列
Matrix4x4 MakeAffineMatrox(const Vector3 &scale, const Vector3 &rotate,
                           const Vector3 &translate) {
  Matrix4x4 result;

  // スケーリング行列
  Matrix4x4 scaleMat = {scale.x, 0, 0,       0, 0, scale.y, 0, 0,
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
  ...
  while (Novice::ProcessMessage() == 0) {
    ...
    Matrix4x4 worldMatrix =
        MakeAffineMatrox({1.0f, 1.0f, 1.0f}, rotate, translate);
    Matrix4x4 cameraMatrix = MakeAffineMatrox(
